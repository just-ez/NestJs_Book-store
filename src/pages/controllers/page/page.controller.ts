import { Controller, Inject, Post, Body, UsePipes, ValidationPipe, Param, HttpException, HttpStatus, Get, Query, ParseIntPipe, Patch } from '@nestjs/common';
...
export class PageController {
   ...
   async findPage (@Query('page') page: string, @Param() params) {
    ...
    async addPage (@Body() pageDto: createPageDto, @Param('') params) {
        ...
        throw new HttpException('No book found you have to create a book to add a page', HttpStatus.BAD_REQUEST)

    }

    @Patch('search/:bookId')
    async updatePage(@Body() pageDto: createPageDto, @Param() params, @Query('page') page: string) {
        const updatePage = await this.pageService.updatePageByPagenumber(pageDto,page,params.bookId)
    ...
    async updatePageById (
        ...
        const updatePage = await this.pageService.updatePageById(params.bookId,params.pageId,pageDto)
